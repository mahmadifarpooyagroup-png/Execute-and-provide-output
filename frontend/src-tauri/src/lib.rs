...
fn start_runtime(app: tauri::AppHandle, state: State<'_, RuntimeProcess>) -> Result<String, String> {
    ...
    if runtime_is_owned(&token_path) { return Ok("already-running".into()); }
    ...
    if let Some(parent) = python.parent() {
        ...
        if let Ok(joined) = std::env::join_paths(paths) { command.env("PATH", joined); }
    }

    let child = command.spawn().map_err(|error| format!("Failed to start Atrin runtime: {error}"))?;
    {
        let mut guard = state.child.lock().map_err(|_| "Runtime process lock is poisoned".to_string())?;
        *guard = Some(child);
    }
...
#[tauri::command]
fn runtime_status(app: tauri::AppHandle, state: State<'_, RuntimeProcess>) -> Result<String, String> {
    let data_dir = app.path().app_local_data_dir().map_err(|error| error.to_string())?;
    let token_path = data_dir.join("runtime_secret.token");
    if runtime_is_owned(&token_path) { return Ok("running".into()); }
    let mut guard = state.child.lock().map_err(|_| "Runtime process lock is poisoned".to_string())?;
    if let Some(child) = guard.as_mut() {
        match child.try_wait() {
            Ok(Some(_)) => { *guard = None; Ok("stopped".into()) }
            Ok(None) => Ok("starting".into()),
            Err(error) => Err(format!("Failed to inspect runtime process: {error}")),
        }
    } else { Ok("stopped".into()) }
}
