...
class WorkflowEngine:
    ...
    def _checkpoint(cls, connection: Any, workflow_id: str, payload: Mapping[str, Any]) -> int:
        ...
        serialized = json.dumps(checkpoint, separators=(",", ":"), sort_keys=True)
        ...
        if row is None:
            ...
            connection.execute(
                ...
                "(workflow_id, checkpoint_version, revision, payload, updated_at) VALUES (?, ?, ?, ?, ?)",
                (workflow_id, int(checkpoint["checkpoint_version"]), revision, serialized, now),
            )
            return revision
        ...
        cursor = connection.execute(
            ...
            (int(checkpoint["checkpoint_version"]), revision, serialized, now, workflow_id, current_revision),
        )
    ...
    def validate_audit_chain(self) -> bool:
        ...
        try:
            rows = connection.execute(
                "SELECT seq,workflow_id,event_type,actor,payload,prev_hash,entry_hash "
                "FROM audit_log ORDER BY seq"
            ).fetchall()
            previous = ""
            for row in rows:
                expected = hashlib.sha256(
                    f"{previous}|{row['workflow_id'] or ''}|{row['event_type']}|{row['actor']}|{row['payload'] or '{}'}".encode()
                ).hexdigest()
        ...
        try:
            ...
            for task_index, task in enumerate(plan):
                ...
                for step_index, step in enumerate(task.steps):
                    ...
                    connection.execute(
                        "INSERT INTO steps "
                        "(step_id,task_id,action,provider_id,idempotency_key,operation_id,status,result,evidence,order_index,provider_profile_id,fencing_token,side_effecting) "
                        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        (step.step_id, task.task_id, step.action, step.provider_id, key, operation_id,
                         status, step.result, step.evidence, step_index, step.provider_profile_id,
                         step.fencing_token, int(bool(step.side_effecting))),
                    )
    ...
    def _assert_step_runnable(self, connection: Any, workflow_id: str, step: Any) -> None:
        ...
        previous_task = connection.execute("""
            ...
            WHERE workflow_id=? AND order_index<? AND status!=?
            ORDER BY order_index LIMIT 1
        """, (workflow_id, step["task_order"], TaskStatus.COMPLETED.value)).fetchone()
        ...
        previous_step = connection.execute("""
            ...
            WHERE task_id=? AND order_index<? AND status!=?
            ORDER BY order_index LIMIT 1
        """, (step["task_id"], step["order_index"], StepStatus.CONFIRMED.value)).fetchone()
    ...
    async def _execute_action(self, adapter: ActionAdapter, action: str, key: str,
                              operation_id: str, fencing_token: int | None) -> Any:
        kwargs: dict[str, Any] = {}
    ...
    async def _cancel_action(self, adapter: ActionAdapter, key: str, operation_id: str | None) -> bool:
        ...
        if not callable(cancel_method):
            return False
        kwargs = {"operation_id": operation_id} if _supports_keyword(cancel_method, "operation_id") else {}
        try:
            return bool(await _call(cancel_method, key, **kwargs))
        except Exception:
            return False
    ...
    async def _finalize_verified_action(self, workflow_id: str, step_id: str, key: str) -> Any:
        ...
        try:
            ...
            connection.execute(
                "UPDATE idempotency_ledger SET status=?, confirmed_at=CURRENT_TIMESTAMP, expires_at=NULL, claim_owner=NULL "
                "WHERE idempotency_key=? AND workflow_id=? AND step_id=?",
                (ExecutionStatus.CONFIRMED.value, key, workflow_id, step_id),
            )
            connection.execute(
                "UPDATE tasks SET status=? WHERE task_id=? AND NOT EXISTS "
                "(SELECT 1 FROM steps WHERE task_id=? AND status!=?)",
    ...
    def _mark_ambiguous(self, workflow_id: str, step_id: str, key: str, reason: str) -> None:
        ...
        try:
            ...
            step = self._step(connection, workflow_id, step_id)
            connection.execute(
                "UPDATE steps SET status=?, result=?, evidence=NULL WHERE step_id=? AND status IN (?, ?, ?)",
                (StepStatus.AMBIGUOUS.value, reason, step_id, StepStatus.EXECUTING.value,
                 StepStatus.FAILED.value, StepStatus.PENDING.value),
            )
            connection.execute(
                "UPDATE idempotency_ledger SET status=?, expires_at=NULL, claim_owner=NULL "
                "WHERE idempotency_key=? AND workflow_id=? AND step_id=?",
                (ExecutionStatus.AMBIGUOUS.value, key, workflow_id, step_id),
            )
            connection.execute(
                "UPDATE workflows SET state=?, updated_at=CURRENT_TIMESTAMP WHERE workflow_id=? AND state!=?",
    ...
    async def execute_step(self, workflow_id: str, step_id: str) -> Any:
        ...
        if verify_before_claim:
            ...
            if verified not in {"NOT_STARTED", "FAILED"}:
                self._mark_ambiguous(workflow_id, step_id, key,
                                     f"Verifier returned {verified} while recovering operation")
        ...
        # Protected side effects require a live exclusive session. If the plan did
        # not provide a token, acquire one atomically through SessionManager before
        # admitting the external action.
        ...
        try:
            ...
            if ledger is None:
                connection.execute(
                    ...
                    "(idempotency_key,workflow_id,step_id,provider_id,operation_id,status,expires_at,claim_owner,attempt) "
                    "VALUES (?,?,?,?,?,?,?,?,1)",
                    (step["idempotency_key"], workflow_id, step_id, step["provider_id"],
                     step["operation_id"], ExecutionStatus.IN_PROGRESS.value, expires, claim_owner),
                )
            elif ledger["status"] in {"FAILED", ExecutionStatus.AMBIGUOUS.value, "PENDING"}:
                cursor = connection.execute(
                    "UPDATE idempotency_ledger SET status=?, operation_id=?, expires_at=?, claim_owner=?, attempt=attempt+1 "
                    "WHERE idempotency_key=? AND workflow_id=? AND step_id=? AND status IN ('FAILED','AMBIGUOUS','PENDING')",
                    (ExecutionStatus.IN_PROGRESS.value, step["operation_id"], expires, claim_owner,
                     step["idempotency_key"], workflow_id, step_id),
                )
                if cursor.rowcount != 1:
                ...
                if verified not in {"NOT_STARTED", "FAILED"}:
                    connection.rollback()
                    self._mark_ambiguous(workflow_id, step_id, step["idempotency_key"],
                                         f"Verifier returned {verified} for expired claim")
                    raise RuntimeError("External action state is ambiguous; workflow paused for recovery")
                cursor = connection.execute(
                    "UPDATE idempotency_ledger SET status=?, operation_id=?, expires_at=?, claim_owner=?, attempt=attempt+1 "
                    "WHERE idempotency_key=? AND workflow_id=? AND step_id=? AND status=? AND expires_at<=?",
                    (ExecutionStatus.IN_PROGRESS.value, step["operation_id"], expires, claim_owner,
                     step["idempotency_key"], workflow_id, step_id, ExecutionStatus.IN_PROGRESS.value, now.isoformat()),
                )
                if cursor.rowcount != 1:
            ...
            if fencing_token is not None:
                connection.execute(
                    "UPDATE steps SET fencing_token=?, operation_id=? WHERE step_id=?",
            ...
            self._audit(connection, workflow_id, "ACTION_CLAIMED", "workflow-engine",
                        {"step_id": step_id, "idempotency_key": step["idempotency_key"],
                         "operation_id": step["operation_id"], "claim_owner": claim_owner})
            connection.commit()
        except Exception:
            connection.rollback()
            raise
        ...
        try:
            if side_effecting and self.session_manager is not None:
                if not self.session_manager.validate_execution_lease(
        ...
        # Unknown failures are deliberately ambiguous: the engine cannot prove the
        ...
        if side_effecting:
            ...
            if verified != "CONFIRMED":
                ...
                self._mark_ambiguous(workflow_id, step_id, key,
                                     f"Verifier returned {verified} after dispatch")
        ...
        try:
            ...
            if current["state"] == WorkflowState.CANCELLED.value:
                ...
                connection.execute(
                    ...
                    (ExecutionStatus.CANCELLED.value if side_effecting and execute_error is None else ExecutionStatus.AMBIGUOUS.value,
            ...
            connection.execute(
                "UPDATE steps SET status=?, result=?, evidence=? WHERE step_id=? AND status=?",
            ...
            task_counts = connection.execute(
                "SELECT COUNT(*) AS total, SUM(CASE WHEN status=? THEN 1 ELSE 0 END) AS done FROM steps WHERE task_id=?",
                (StepStatus.CONFIRMED.value, step["task_id"]),
            ).fetchone()
            if task_counts["total"] and task_counts["done"] == task_counts["total"]:
                connection.execute("UPDATE tasks SET status=? WHERE task_id=?",
                                   (TaskStatus.COMPLETED.value, step["task_id"]))
            remaining = connection.execute(
                "SELECT COUNT(*) AS n FROM tasks WHERE workflow_id=? AND status!=?",
                (workflow_id, TaskStatus.COMPLETED.value),
            ).fetchone()["n"]
    ...
    def _mark_failed(self, workflow_id: str, step_id: str, key: str, result: str, evidence: Any) -> None:
        ...
        try:
            ...
            connection.execute(
                "UPDATE idempotency_ledger SET status='FAILED', expires_at=NULL, claim_owner=NULL "
                "WHERE idempotency_key=? AND workflow_id=? AND step_id=?",
                (key, workflow_id, step_id),
            )
            connection.execute(
            ...
            self._checkpoint(connection, workflow_id, {
                ...
                "state": WorkflowState.FAILED.value, "action_idempotency_key": key,
                "last_result": result, "evidence": evidence,
            })
        ...
        if "step_id" not in checkpoint:
            ...
            try:
                pending_step = connection.execute("""
                    SELECT s.step_id,s.task_id,s.action,s.provider_id,s.idempotency_key,s.operation_id,
            ...
            if pending_step is not None:
                checkpoint.update({"step_id": pending_step["step_id"], "task_id": pending_step["task_id"],
                                   ...
                                   "operation_id": pending_step["operation_id"],
    ...
    async def resume_workflow(self, workflow_id: str, checkpoint: Mapping[str, Any] | None = None,
        ...
        try:
            ...
            checkpoint.update({"provider_id": step["provider_id"],
            ...
            if skip_action:
                ...
                connection.execute("UPDATE steps SET status=? WHERE step_id=?",
                                   (StepStatus.CONFIRMED.value, step["step_id"]))
                connection.execute(
                    "UPDATE tasks SET status=? WHERE task_id=? AND NOT EXISTS "
                    "(SELECT 1 FROM steps WHERE task_id=? AND status!=?)",
                    (TaskStatus.COMPLETED.value, step["task_id"], step["task_id"], StepStatus.CONFIRMED.value),
    ...
    async def cancel_workflow(self, workflow_id: str) -> None:
        ...
        try:
            ...
            connection.execute("UPDATE workflows SET state=?, updated_at=CURRENT_TIMESTAMP WHERE workflow_id=?",
                               (WorkflowState.CANCELLING.value if hasattr(WorkflowState, "CANCELLING") else WorkflowState.RECOVERING.value, workflow_id))
            connection.commit()
        except Exception:
            connection.rollback()
            raise
