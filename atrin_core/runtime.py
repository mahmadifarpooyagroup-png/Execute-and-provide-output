...
def get_security_manager(token_file_path: str = TOKEN_PATH) -> LocalSecurityManager:
    ...
    try:
        if state:
            rows = connection.execute(
                "SELECT workflow_id, goal, state, plan_version, created_at, updated_at "
                "FROM workflows WHERE state=? ORDER BY updated_at DESC",
                (state,),
            ).fetchall()
        else:
            rows = connection.execute(
...
async def get_workflow(workflow_id: str, authenticated: bool = Depends(require_auth)):
    ...
    try:
        ...
        steps = connection.execute(
            ...
            "WHERE t.workflow_id=? ORDER BY t.order_index,s.order_index",
            (workflow_id,),
        ).fetchall()
    ...
    try:
        if workflow_id:
            rows = connection.execute(
                ...
                "FROM audit_log WHERE workflow_id=? ORDER BY seq DESC LIMIT ?",
                (workflow_id, limit),
            ).fetchall()
        else:
            rows = connection.execute(
