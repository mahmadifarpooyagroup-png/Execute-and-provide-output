...
async def test_cpu_quota_kills_busy_loop_process():
    ...
    action = ExecutionAction(
        ...
        execution_target=ExecutionTarget.PYTHON,
        arguments=["-c", "x = 0\nwhile True:\n    x += 1"],
        timeout_seconds=30,  # generous outer bound — the CPU quota should fire first
        working_dir=None,
    )
    result = await bus.execute(action, lambda: True)
    # Killed by SIGXCPU/SIGKILL -> non-zero/negative exit code, not "completed"
    assert result.status != "completed"
...
async def test_memory_quota_kills_unbounded_allocation():
    ...
    bus = ExecutionBus(max_memory_bytes=64 * 1024 * 1024)  # 64 MB
    action = ExecutionAction(
        action_id="memory-hog",
        permission_required=PermissionLevel.EXECUTE_SAFE,
        execution_target=ExecutionTarget.PYTHON,
        arguments=["-c", "data = bytearray(500 * 1024 * 1024)"],  # 500 MB >> 64 MB quota
        timeout_seconds=15,
        working_dir=None,
    )
    result = await bus.execute(action, lambda: True)
...
async def test_default_quotas_do_not_break_normal_execution():
    ...
    bus = ExecutionBus()  # uses default max_memory_bytes=512MB, max_child_processes=16
    action = ExecutionAction(
