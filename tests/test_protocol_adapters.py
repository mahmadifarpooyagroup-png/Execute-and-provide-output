...
async def test_mcp_adapter_stateless_rpc_lifecycle_and_tool_calls():
    ...
    adapter = MCPAdapter(
        MCPConfig(server_url="https://example.test", capabilities=["tools"], auth_token="token-123"),
        client=mock_client,
    )
...
async def test_a2a_adapter_jsonrpc_task_lifecycle():
    ...
    mock_client.get = AsyncMock(side_effect=[
        ...
        FakeResponse(json_data={"id": "task-123", "status": "completed"}),
    ])
    mock_client.post = AsyncMock(return_value=FakeResponse(json_data={
        "jsonrpc": "2.0", "id": 1, "result": {"id": "task-123", "status": "working"}
    }))
